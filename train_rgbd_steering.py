# Modified the get_action function. Included compute_trajectory_scores as well.
...
class Agent(nn.Module):
    ...
    def get_action(self, obs_seq, num_particles=10):
        B = obs_seq["state"].shape[0]
        with torch.no_grad():
            # process the observations as in your original code
            if self.include_rgb:
            ...
            # sample multiple trajectories (particles)
            all_action_sequences = []
            for _ in range(num_particles):
                # Initialize from different noise seeds
                ...
                all_action_sequences.append(noisy_action_seq)
            
            # stack all sequences for batch evaluation
            all_action_sequences = torch.stack(all_action_sequences, dim=0)
            
            # score each trajectory using a reward function
            scores = self.compute_trajectory_scores(all_action_sequences, obs_seq)
            
            # select the best trajectory
            best_idx = torch.argmax(scores, dim=0)
            best_actions = all_action_sequences[best_idx, torch.arange(B)]
            
            # return the action window as in original code
            start = self.obs_horizon - 1
            end = start + self.act_horizon
            return best_actions[:, start:end]

    def compute_trajectory_scores(self, action_sequences, obs_seq):
        # action_sequences: [num_particles, batch_size, pred_horizon, act_dim]
        num_particles, B = action_sequences.shape[:2]
        scores = torch.zeros((num_particles, B), device=action_sequences.device)
        
        # Simple scoring: reward action smoothness (less jerky motions)
        for i in range(num_particles):
            actions = action_sequences[i]  # [batch_size, pred_horizon, act_dim]
            
            # Calculate action differences (penalize large changes)
            action_diffs = torch.diff(actions, dim=1)
            smoothness = -torch.mean(torch.norm(action_diffs, dim=2), dim=1)
            scores[i] = smoothness
